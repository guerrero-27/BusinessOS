<x-app-layout>
    <div class="max-w-2xl mx-auto">
        <div class="mb-6 flex items-center gap-3">
            ...
            <h1 class="text-xl font-semibold text-[#111111]">Add Customer</h1>
        </div>

        <form action="{{ route('customers.store') }}" method="POST" class="rounded-2xl border border-black/10 bg-white p-6 shadow-sm space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea name="address" rows="2"
                          class="w-full rounded-lg border border-black/10 shadow-sm text-sm focus:border-[#4CAF50] focus:ring-[#4CAF50]">{{ old('address') }}</textarea>
                @error('address')
                    <p class="text-xs text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
